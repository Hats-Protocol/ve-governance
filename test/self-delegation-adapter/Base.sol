...
import {FixedPointBase} from "../v1_4_0/base/FixedPointBase.sol";

contract EscrowVotingPowerMock is IDelegateUpdateVotingPower {
    function updateVotingPower(address a, address b) external {}
}

contract SelfDelegationEscrowIVotesAdapterA is SelfDelegationEscrowIVotesAdapter {
    constructor(
        int256[3] memory coefficients,
        uint256 maxEpoch
    ) SelfDelegationEscrowIVotesAdapter(coefficients, maxEpoch) {}
    ...
    Test
{
    ...
    SelfDelegationEscrowIVotesAdapterA public dg;
    DAO dao;
    Clock clock;
    ...
    function _deployEscrowIVotesAdapter(
        ...
        address _clock,
        address _escrow
    ) public returns (SelfDelegationEscrowIVotesAdapterA) {
        (int256[3] memory coefficients, uint256 maxEpochs) = CurveConstantLib.getCoefficients();
        SelfDelegationEscrowIVotesAdapterA impl = new SelfDelegationEscrowIVotesAdapterA(coefficients, maxEpochs);
        ...
        return SelfDelegationEscrowIVotesAdapterA(address(impl).deployUUPSProxy(initCalldata));
    }
